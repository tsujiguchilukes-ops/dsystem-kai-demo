...
eq('★全営業日の給率が式①と一致', rateNG.length, 0, 'ログ148/156');
if (rateNG.length) NG[NG.length - 1].label += '（' + rateNG.slice(0, 6).join(' / ') + '）';
...
/* ============ 11. 仮で埋めた値の検算 ============
 * 本家の画面が無く観測できなかったところを、想像で埋めた。
 * 想像でも「観測できている数字と矛盾しない」ことは機械で守る。 */
// キャスト×商品：商品ごとの合計が月間の観測値と一致するか
(function () {
  const g = D.castItemGrid || {};
  const casts = Object.keys(g);
  let ng = [];
  Object.keys(D.itemTotals).forEach(function (p) {
    const sum = casts.reduce(function (a, c) { return a + (g[c][p] || 0); }, 0);
    if (sum !== D.itemTotals[p]) ng.push(p + ' ' + sum + '/' + D.itemTotals[p]);
  });
  eq('★仮:キャスト×商品の合計が月間と一致', ng.length, 0, '観測ログ263-266');
  if (ng.length) NG[NG.length - 1].label += '（' + ng.join(' / ') + '）';
  // キャストドリンクSの人別本数は観測値そのもの
  const CDS = { 'べる☆': 123, 'しずく🌙': 172, 'はるか🌻': 83, 'ゆき❄️': 49, 'リリ☆': 47,
                'みずき☆': 46, 'りく🎣': 44, 'りん🔔': 36, 'らん🍖': 17, 'スタッフ': 2 };
  let cdsNg = [];
  Object.keys(CDS).forEach(function (c) {
    if (!g[c] || g[c]['キャストドリンクS'] !== CDS[c]) cdsNg.push(c);
  });
  eq('★仮:キャストドリンクSの人別本数は観測どおり', cdsNg.length, 0, '観測ログ271-272');
  // 6名の総個数は観測値どおりか
  const TOT = { 'らん🍖': 17, 'はるか🌻': 127, 'しずく🌙': 286, 'みずき☆': 57, 'リリ☆': 65, 'りん🔔': 37 };
  let totNg = [];
  Object.keys(TOT).forEach(function (c) {
    const n = Object.keys(g[c] || {}).reduce(function (a, k) { return a + g[c][k]; }, 0);
    if (n !== TOT[c]) totNg.push(c + ' ' + n + '/' + TOT[c]);
  });
  eq('★仮:6名の総個数は観測どおり', totNg.length, 0, '観測ログ310-313');
  if (totNg.length) NG[NG.length - 1].label += '（' + totNg.join(' / ') + '）';
})();
// 月次補正の内訳は合計15,000か
eq('★仮:月次補正の内訳の合計',
   (D.store.monthlyAdjustDetail || []).reduce(function (a, x) { return a + x.amount; }, 0),
   D.store.monthlyAdjust, '観測ログ68/167から逆算');
// 粗利算入フラグが効くか（粗利×の分がちゃんと除かれるか）
(function () {
  const t = C.flowTotals(D.cashFlows);
  eq('★仮:粗利に入らない入出金が除かれる', t.excludedGross, 200000, '観測ログ505-515');
  eq('★仮:現金に入らない入出金が除かれる', t.excludedCash, 50000, '観測ログ505-515');
  // 粗利×だけを置いたので、月間粗利は動かないはず
  eq('★仮:入出金を足しても月間粗利は不変', agg.grossProfit, 1790511, '本家との一致を壊していないか');
})();
// キャスト個別バックを入れても 8/24 の1円一致が崩れていないか
(function () {
  const withBack = D.casts.filter(function (c) { return c.back; }).length;
  eq('★仮:キャスト個別バックが入っている人数', withBack > 0, true, '観測ログ308');
  const on0824 = ['ゆき❄️', 'しずく🌙', 'りん🔔'];
  const bad = on0824.filter(function (n) {
    const c = D.casts.find(function (x) { return x.name === n; }); return c && c.back;
  });
  eq('★8/24の3名は店舗設定のまま（1円一致を守るため）', bad.length, 0, '検算の保護');
})();

// 給率の分子・分母の既定値。観測（月33.45%・日別4件）と一致するのはこの組み合わせだけなので確定扱いにする
eq('給率の既定は 分子=残り支給額+女子日払い', D.store.payRateNumerator, 'remainPlusFemaleDaily', 'ログ148/156');
eq('給率の既定は 分母=売上計',                D.store.payRateDenominator, 'salesTotal', 'ログ148/156');

/* ============ 12. まだ式を一意に決められないもの（実機で何を見れば決まるか） ============ */
