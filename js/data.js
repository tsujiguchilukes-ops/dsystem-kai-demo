...
(function (global) {
  ...
  const store = {
    ...
    monthlyAdjust: 15000,
    // 15,000の内訳（★仮の値）。合計15,000であることだけが観測から確定していて、
    // どれが幾らかは本家のキャスト画面が無いため分からない。よくある形で分けた。
    monthlyAdjustDetail: [
      { name: "キャスト調整額", amount: 10000, note: "月末のならし・特別手当など" },
      { name: "キャスト月給",   amount: 5000,  note: "日給とは別に月額で払う分" },
      { name: "スタッフ月給",   amount: 0,     note: "今月は該当なし" },
      { name: "ホステス源泉徴収", amount: 0,   note: "この店は使っていない設定" },
    ],
    name: "草加 Lua",
    openHour: 16,                 // 営業日境界
    ...
    // ---- 2部時給の開始時刻（★仮の値。本家の設定画面が未観測）----
    // 夜の店は深夜から単価が上がるのが普通。24:00（＝翌0時）から2部にした
    part2Hour: "24:00",
  ...
  const casts = [
    // back: キャスト個別の指名バック（★仮の値。空なら店舗設定に従う）
    //   本家は「リクエスト/同伴/場内はキャスト個別のバック率」（観測ログ308行）だが、
    //   個別の数値は画面が無くて未観測。売れっ子ほど手厚い、という現場の常識で置いた。
    //   ⚠️ ゆき/しずく/りん は 8/24 の勤怠で本家と1円一致を検算しているため、あえて店舗設定のまま。
    // wage2: 2部時給（★仮の値）。店舗設定の part2Hour を過ぎた分に適用
    { id: 110, name: "べる☆",   attr: "normal",   wage1: 2100, wage2: 2600, welfare: 10,
      back: { req: { mode: "fixed", value: 800 }, dohan: { mode: "fixed", value: 2500 } } },
    { id: 180, name: "しずく🌙", attr: "normal",   wage1: 2000, wage2: 2500, welfare: 10 },
    { id: 213, name: "はるか🌻", attr: "dispatch", wage1: 2000, wage2: 2400, welfare: 10,
      back: { req: { mode: "fixed", value: 700 }, dohan: { mode: "fixed", value: 2500 } } },
    { id: 238, name: "みずき☆", attr: "normal",   wage1: 1800, wage2: 2200, welfare: 10,
      back: { req: { mode: "fixed", value: 500 } } },
    { id: 131, name: "リリ☆",   attr: "normal",   wage1: 1900, wage2: 2300, welfare: 10,
      back: { req: { mode: "fixed", value: 600 }, dohan: { mode: "fixed", value: 2200 } } },
    { id: 241, name: "りん🔔",   attr: "normal",   wage1: 1600, wage2: 2000, welfare: 10 },
    { id: 187, name: "ゆき❄️",   attr: "normal",   wage1: 1800, wage2: 2200, welfare: 10 },
    { id: 244, name: "りく🎣",   attr: "normal",   wage1: 1700, wage2: 2100, welfare: 10,
      back: { req: { mode: "fixed", value: 500 } } },
    { id: 245, name: "れん🌹",   attr: "normal",   wage1: 1600, wage2: 2000, welfare: 10 },
    { id: 230, name: "らん🍖",   attr: "normal",   wage1: 1500, wage2: 1900, welfare: 10,
      back: { req: { mode: "fixed", value: 400 } } },
  ];
  ...
  // ---- キャスト×商品の実数（★一部は仮の値）----
  // 観測できたのは「キャストドリンクSの人別本数」と「6名の総個数」まで。
  // それ以外の内訳は本家の画面が無いため、次の2つを必ず満たすように埋めた仮の値:
  //   ① 商品ごとの合計＝月間の観測値と完全一致  ② 6名の非CDS個数＝観測値と完全一致
  // ⚠️ 販売価格・バック額の人別合計は観測値と差が出る（今わかっている単価だけでは説明できないため）。
  //    本家の「キャスト別商品集計」を1枚撮れば確定できる。
  const castItemGrid = {
    "べる☆": { "キャストドリンクS": 123, "コカレロ": 10, "クライナー": 9, "テキーラショット各種": 5 },
    "しずく🌙": { "キャストドリンクS": 172, "テキーラショット各種": 45, "クライナー": 34, "コカレロ": 26, "フード800": 4, "ヴーヴ白": 2, "ジャックダニエル": 2, "★キャストショット": 1 },
    "はるか🌻": { "キャストドリンクS": 83, "テキーラショット各種": 16, "クライナー": 14, "コカレロ": 10, "フード800": 3, "ジャックダニエル": 1 },
    "みずき☆": { "キャストドリンクS": 46, "クライナー": 9, "テキーラショット各種": 1, "コカレロ": 1 },
    "リリ☆": { "キャストドリンクS": 47, "クライナー": 9, "テキーラショット各種": 6, "ヴーヴロゼ": 2, "モエN.I.R": 1 },
    "りん🔔": { "キャストドリンクS": 36, "クライナー": 1 },
    "ゆき❄️": { "キャストドリンクS": 49, "クライナー": 6, "コカレロ": 6, "テキーラショット各種": 3 },
    "りく🎣": { "キャストドリンクS": 44, "クライナー": 5, "コカレロ": 5, "テキーラショット各種": 3 },
    "らん🍖": { "キャストドリンクS": 17 },
    "スタッフ": { "クライナー": 3, "コカレロ": 3, "キャストドリンクS": 2, "テキーラショット各種": 1 },
  };
  ...
  // ---- 入出金の実レコード（★仮の値）----
  // 本家は入出金項目ごとに「粗利に入れるか」「現金に入れるか」の2フラグを持つ（観測ログ505-515行）。
  // 今月の観測データには粗利×のレコードが1件も無く、フラグが効くところを確かめられなかった。
  // そこで、粗利に入らない出金／入金を1件ずつ置いた。
  // ⚠️ どちらも粗利×なので、月間粗利1,790,511は変わらない（本家との1円一致は保たれる）。
  const cashFlows = [
    { date: "2026-08-05", itemId: 13531, name: "テナント",       amount: 0,      io: "out", kind: "out_cash_gross"     },
    { date: "2026-08-12", itemId: 17549, name: "交際費",         amount: 30000,  io: "out", kind: "out_cash_nogross"   },
    { date: "2026-08-20", itemId: 13532, name: "補償金",         amount: 50000,  io: "out", kind: "out_nocash_nogross" },
    { date: "2026-08-25", itemId: 65827, name: "カード前期入金", amount: 120000, io: "in",  kind: "in_monthly"         },
  ];
  ...
  const F = [
    ...
    ["2026-08-02","日",0, 59000, 0, 12600, 71600, 0, 0, 20110, 42.05, 14400, 10000, 0, 500, 0, 0, 44510, 27090],
    ["2026-08-03","月",0, 102800,0,     0, 102800,  85600, 13900,  27580,31.69,     0, 5000,0,   0,0,  500,  33080,  69720],
    ["2026-08-04","火",0, 82700, 0, 0, 82700, 57200, 57200, 22710, 33.51, 11000, 5000, 0, 0, 0, 0, 38710, 43990],
    ["2026-08-05","水",0, 26900, 0, 100400, 127300, 106000, 0, 28200, 26.08, 10000, 5000, 0, 0, 0, 0, 43200, 84100],
    ["2026-08-06","木",0, 19300, 0, 112200, 131500, 89800, 39900, 36915, 33.4, 11100, 7000, 0, 0, 0, 0, 55015, 76485],
    ...
    ["2026-08-13","木",0, 65100, 0, 7600, 72700, 47900, 27300, 22597, 40.71, 11500, 7000, 0, 500, 0, 0, 41097, 31603],
    ...
    ["2026-08-17","月",0, 15500, 0, 63000, 78500, 65400, 0, 34315, 52.63, 0, 7000, 0, 500, 0, 0, 41315, 37185],
    ...
    ["2026-08-21","金",0, 61100, 0, 244300, 305400, 181400, 0, 80580, 29.66, 16000, 10000, 0, 500, 0, 15800, 122380, 183020],
    ["2026-08-22","土",0, 172100, 0, 14200, 186300, 123100, 48200, 48768, 32.62, 11000, 12000, 0, 1000, 0, 0, 71768, 114532],
  ...
  global.DATA = {
    ...
    store, terms, t, casts, staff, products, castItemGrid, costItems, cashFlows, feeItems, tags,
